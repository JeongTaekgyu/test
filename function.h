...
void move_block(int dir, Block_Info * block_info, Map_Info * map_info) { //블록을 이동시킴 
	...
	case LEFT: //왼쪽방향 
		for (i = 0; i < 4; i++) { //현재좌표의 블럭을 지움 
			for (j = 0; j < 4; j++) {
				if (blocks[(*block_info).b_type][(*block_info).b_rotation][i][j] == 1) 
					(*map_info).main_org[(*block_info).by + i][(*block_info).bx + j] = EMPTY;
			}
		}
		for (i = 0; i < 4; i++) { //왼쪽으로 한칸가서 active block을 찍음 
			for (j = 0; j < 4; j++) {
				if (blocks[(*block_info).b_type][(*block_info).b_rotation][i][j] == 1) 
					(*map_info).main_org[(*block_info).by + i][(*block_info).bx + j - 1] = ACTIVE_BLOCK;
			}
		}
	...
	case DOWN:    //아래쪽 방향. 왼쪽방향이랑 같은 원리로 동작
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				if (blocks[(*block_info).b_type][(*block_info).b_rotation][i][j] == 1) 
					(*map_info).main_org[(*block_info).by + i + 1][(*block_info).bx + j] = ACTIVE_BLOCK;
			}
		}
		(*block_info).by++;
		break;
	...
	case UP: //키보드 위쪽 눌렀을때 회전시킴. 
		...
		for (i = 0; i < 4; i++) { //회전된 블록을 찍음 
			for (j = 0; j < 4; j++) {
				if (blocks[(*block_info).b_type][(*block_info).b_rotation][i][j] == 1) 
					(*map_info).main_org[(*block_info).by + i][(*block_info).bx + j] = ACTIVE_BLOCK;
			}
		}
		break;
